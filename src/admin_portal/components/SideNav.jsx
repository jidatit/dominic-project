...
const SideNav = () => {
  ...
  const menus = [
    ...
    { name: "Questions", link: "/admin_portal/questions", icon: BsQuestionSquare },
  ...
  return (
    <section className="flex">
      <div
        className={` bg-[#6D00F8] min-h-screen shadow-md ${open ? "w-68" : " w-14 lg:w-[75px]"
        ...
        <div className="mt-4 flex flex-col gap-4 relative">
          {menus?.map((menu, i) => (
            <Link
              to={menu?.link}
              key={i}
              className={`group flex items-center text-base gap-2 font-poppins ${open && "py-2 pl-2 pr-4"} ${location.pathname === menu?.link ? "bg-[rgba(112,127,221,0.1) text-[#ffffff] bg-[#4179d1] rounded-md" : "text-white"} hover:bg-[rgba(112,127,221,0.1)] hover:text-[#FFFFFF] rounded-md`}
            >
