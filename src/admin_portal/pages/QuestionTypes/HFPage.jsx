...
const HFPage = () => {
  const [HFQuestionsData, setHFQuestionsData] = useState([]);
  ...
  const handleSaveChanges = async () => {
    try {
      if (editedQuestion.length > 5) {
        const docRef = doc(db, 'HFQuestions', selectedRowData.id);
        await updateDoc(docRef, {
          question: editedQuestion
        });
        const updatedData = HFQuestionsData.map(item =>
          item.id === selectedRowData.id ? { ...item, question: editedQuestion } : item
        );
        setHFQuestionsData(updatedData);
        setIsModalOpen(false);
      }
      else {
  ...
  const handleAddQuestion = () => {
    if (HFQuestionsData.length < MAX_QUESTIONS) {
  ...
  const handleSaveNewQuestion = async () => {
    try {
      if (NewQuestion.length > 5) {
        const docRef = await addDoc(collection(db, 'HFQuestions'), { question: NewQuestion });
        const newQuestion = { id: docRef.id, question: NewQuestion };
        setHFQuestionsData([...HFQuestionsData, newQuestion]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const HFcollection = collection(db, 'HFQuestions');
        const snapshot = await getDocs(HFcollection);
        const Data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        setHFQuestionsData(Data);
      } catch (error) {
