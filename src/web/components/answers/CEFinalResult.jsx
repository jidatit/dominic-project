...
const CEFinalResult = () => {
    ...
    const fetchUsersAnswers = async () => {
        try {
            const answersDocRef = doc(db, 'CEResponses', user.uid);
            ...
            {user ? (
                <div id="result-content" className="w-full h-auto min-h-[520px] flex flex-col gap-8 border-t-2 border-gray-300 py-8">
                    ...
                    <div className="w-full flex lg:flex-row flex-col justify-evenly items-center gap-2 lg:px-44 px-8">
                        <button onClick={handlePrint} className="lg:w-auto w-full flex flex-row justify-center items-center gap-2 py-3 px-6 bg-[#2187A2] text-white cursor-pointer rounded-md">
                            <p className="text-sm lg:text-lg font-bold"> Print </p>
                            <PrintIcon />
                        </button>
                        <button onClick={handleSaveAsPDF} className="lg:w-auto w-full flex flex-row justify-center items-center gap-2 py-3 px-6 bg-[#2187A2] text-white cursor-pointer rounded-md">
                    ...
                    {healthAndFitnessQuestions.map((question, index) => (
                        <div key={index} className="w-full flex flex-col justify-start items-start lg:px-44 px-8">
                            <h2 className="text-base font-semibold">Question # {" "}{index + 1}{" : "}{question.question}</h2>
                            <div className='w-full flex flex-row justify-start items-start gap-4' >
                                <p className="text-base font-semibold" > Answer {" : "} </p>
                                <p className="text-base text-[#03009C] font-bold">Current Level: {usersAnswers[index]?.current || 'Not Answered'}</p>
                                <p className="text-base text-[#03009C]  font-bold">Ideal Level: {usersAnswers[index]?.desired || 'Not Answered'}</p>
                            </div>
                        </div>
                    ))}
                </div>
