					...
					{introductionState === true ? (
						<>
							...
							<p className="w-full text-lg text-center lg:px-44 px-8">
								Imagine a canvas where your physical activities, dietary
								choices, and rest patterns paint the picture of your well-being.
								It's a personal journey that varies widely among individuals,
								where each has their unique rhythm and definition of vitality.
							</p>
							<div className="w-full flex justify-end items-center lg:px-44 px-8">
								<button
									onClick={handleStart}
									className="w-40 flex flex-row justify-center items-center gap-2 py-3 px-6 bg-[#6D00F8] text-white cursor-pointer rounded-md"
								>
											...
											{[...Array(10)].map((_, i) => (
												<div
													...
													className={`${classForCurrentState(i + 1)} ${
														i === 0
															? "border-2 border-customPurple rounded-tl-lg rounded-bl-lg"
															: i === 9
																? "border-t-2 border-b-2 border-r-2 border-customPurple rounded-tr-lg rounded-br-lg"
																: "border-t-2 border-b-2 border-r-2 border-customPurple"
													}`}
												>
													{i + 1}
												</div>
